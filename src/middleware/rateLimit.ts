...
export function rateLimit(config: Partial<RateLimitConfig> = {}) {
	...
	return async (c: Context, next: Next) => {
		...
		// Use the last IP in X-Forwarded-For as it's the one that connected to our trusted proxy (Caddy)
		// The first IP can be spoofed by the client sending their own X-Forwarded-For header
		const ip =
			forwardedFor?.split(',').pop()?.trim() || c.req.header('x-real-ip') || 'unknown'
