...
export interface RateLimitConfig {
	...
	scope?: string // Scope to isolate rate limits (e.g. 'strict', 'moderate')
}
...
export function rateLimit(config: Partial<RateLimitConfig> = {}) {
	...
	return async (c: Context, next: Next) => {
		...
		// Add scope to key to prevent collisions between different limiters
		if (finalConfig.scope) {
			key = `${key}:${finalConfig.scope}`
		}
