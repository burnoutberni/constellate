...
export interface RateLimitConfig {
	...
	scope?: string // Optional scope to isolate different limiters (e.g., "login", "search")
}
...
export function rateLimit(config: Partial<RateLimitConfig> = {}) {
	...
	return async (c: Context, next: Next) => {
		...
		// Append scope if provided to avoid collisions between different limiters
		if (finalConfig.scope) {
			key += `:${finalConfig.scope}`
		}
