...
function buildRecommendation(
	...
	if (attendanceCount >= MIN_ATTENDANCE_FOR_POPULARITY_REASON) {
		reasons.push(`Already ${attendanceCount} people attending`)
	}
	if (reasons.length === 0) {
...
export async function getEventRecommendations(userId: string, limit?: number) {
	...
	const visibilityWhere = buildVisibilityWhere({
		userId,
		followedActorUrls: Array.from(followedActorUrls),
	})
	const now = new Date()
