...
describe('Search API - Nearby events', () => {
	...
	it('returns events sorted by distance within the requested radius', async () => {
		...
		const response = await app.request(
			'/api/search/nearby?latitude=40.7128&longitude=-74.0060&radiusKm=200&limit=5'
		)
		expect(response.status).toBe(200)

		const body = (await response.json()) as {
			events: Array<{ id: string; distanceKm: number }>
		}
	...
	it('excludes events outside the requested radius', async () => {
		...
		const response = await app.request(
			'/api/search/nearby?latitude=40.7812&longitude=-73.9665&radiusKm=10'
		)
		expect(response.status).toBe(200)
	...
	it('rejects searches at extreme latitudes (beyond ±85°)', async () => {
		const response1 = await app.request(
			'/api/search/nearby?latitude=86&longitude=0&radiusKm=10'
		)
	...
	it('rejects invalid radius values exceeding schema limits', async () => {
		...
		const response = await app.request(
			'/api/search/nearby?latitude=0&longitude=0&radiusKm=1000'
		)
	...
	it('rejects invalid latitude values', async () => {
		const response1 = await app.request(
			'/api/search/nearby?latitude=91&longitude=0&radiusKm=10'
		)
		expect(response1.status).toBe(400)
		...
		const response3 = await app.request(
			'/api/search/nearby?latitude=invalid&longitude=0&radiusKm=10'
		)
	...
	it('rejects invalid longitude values', async () => {
		const response1 = await app.request(
			'/api/search/nearby?latitude=0&longitude=181&radiusKm=10'
		)
		expect(response1.status).toBe(400)
		...
		const response3 = await app.request(
			'/api/search/nearby?latitude=0&longitude=invalid&radiusKm=10'
		)
		expect(response3.status).toBe(400)
	})
	...
	it('excludes events without coordinates (null locationLatitude or locationLongitude)', async () => {
		...
		const response = await app.request(
			'/api/search/nearby?latitude=40.7128&longitude=-74.0060&radiusKm=50'
		)
		expect(response.status).toBe(200)
	...
	it('only returns public events for unauthenticated requests', async () => {
		...
		const response = await app.request(
			'/api/search/nearby?latitude=40.7128&longitude=-74.0060&radiusKm=10'
		)
		expect(response.status).toBe(200)
	...
	it('rejects searches with radius too large for the location', async () => {
		...
		const response = await app.request(
			'/api/search/nearby?latitude=80&longitude=0&radiusKm=20000'
		)
		expect(response.status).toBe(400)
		...
		for (let i = 0; i < 10; i++) {
			await prisma.event.create({
				data: {
					...
					locationLongitude: -74.006 + i * 0.001,
				},
			})
		}

		const response = await app.request(
			'/api/search/nearby?latitude=40.7128&longitude=-74.0060&radiusKm=50&limit=5'
		)
		expect(response.status).toBe(200)
	...
	it('returns origin metadata in response', async () => {
		...
		const response = await app.request(
			'/api/search/nearby?latitude=40.7128&longitude=-74.0060&radiusKm=25&limit=10'
		)
		expect(response.status).toBe(200)
		...
		expect(body.origin.longitude).toBe(-74.006)
	...
	it('sorts events by distance (closest first)', async () => {
		...
		const closeEvent = await prisma.event.create({
			data: {
				...
				locationLatitude: 40.713, // Very close
		...
		const farEvent = await prisma.event.create({
			data: {
				...
				locationLatitude: 40.72, // Further away
				locationLongitude: -74.01,
			},
		})
	...
	it('finds events on both sides of the antimeridian when crossing from east (lonMax > 180)', async () => {
		...
		const response = await app.request(
			'/api/search/nearby?latitude=0&longitude=179&radiusKm=250'
		)
		expect(response.status).toBe(200)
	...
	it('correctly filters by Haversine distance when crossing antimeridian', async () => {
		...
		const response = await app.request(
			'/api/search/nearby?latitude=0&longitude=179&radiusKm=100'
		)
		expect(response.status).toBe(200)
