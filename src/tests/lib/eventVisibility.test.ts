...
import { buildVisibilityWhere, canUserViewEvent, resolveEventActorUrl, isPublicVisibility } from '../../lib/eventVisibility.js'
...
describe('eventVisibility helpers', () => {
    ...
    describe('canUserViewEvent', () => {
        it('allows public and unlisted events for everyone', async () => {
            await expect(
                canUserViewEvent({ visibility: 'PUBLIC' }, undefined)
            ).resolves.toBe(true)
            await expect(
                canUserViewEvent({ visibility: 'UNLISTED' }, undefined)
            ).resolves.toBe(true)
        })
