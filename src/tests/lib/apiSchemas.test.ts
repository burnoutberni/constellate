...
describe('API Schemas', () => {
	...
	describe('ProfileUpdateSchema', () => {
		it('should parse valid profile update', async () => {
			const valid = {
				name: 'Alice Smith',
			...
			const result = await ProfileUpdateSchema.safeParseAsync(valid)
			expect(result.success).toBe(true)
		})

		it('should parse partial profile update', async () => {
			const valid = { name: 'Alice Smith' }
			const result = await ProfileUpdateSchema.safeParseAsync(valid)
			expect(result.success).toBe(true)
		})

		it('should parse empty profile update', async () => {
			const valid = {}
			const result = await ProfileUpdateSchema.safeParseAsync(valid)
			expect(result.success).toBe(true)
		})
		...
		it('should reject name exceeding max length', async () => {
			...
			const result = await ProfileUpdateSchema.safeParseAsync(invalid)
			expect(result.success).toBe(false)
		})
		...
		it('should reject invalid display color format', async () => {
			const invalid = { displayColor: 'not-a-color' }
			const result = await ProfileUpdateSchema.safeParseAsync(invalid)
			expect(result.success).toBe(false)
		})

		it('should reject display color without hash', async () => {
