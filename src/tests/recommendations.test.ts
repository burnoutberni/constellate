...
describe('Event recommendations API', () => {
    ...
    it('handles events with only attributedTo (no userId)', async () => {
        ...
        const hasRecommendedEvent = body.recommendations.some((r) => r.event.id === recommendedEvent.id)
    ...
    it('includes host affinity when user engaged with host before', async () => {
        ...
        if (recommendation) {
            expect(recommendation.reasons.some((r) => r.includes('engaged with this host'))).toBe(true)
        }
    })
    ...
    it('prioritizes events from followed organizers', async () => {
        ...
        expect(response.status).toBe(200)
        const followedEventIndex = body.recommendations.findIndex((r) => r.event.id === eventFromFollowed.id)
        const otherEventIndex = body.recommendations.findIndex((r) => r.event.id === eventFromOther.id)

        if (followedEventIndex >= 0 && otherEventIndex >= 0) {
            expect(followedEventIndex).toBeLessThan(otherEventIndex)
        }

        const followedRecommendation = body.recommendations.find((r) => r.event.id === eventFromFollowed.id)
        if (followedRecommendation) {
            expect(followedRecommendation.reasons.some((r) => r.includes('someone you follow'))).toBe(true)
        }
    })
