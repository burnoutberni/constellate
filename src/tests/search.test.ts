...
describe('Search API - Advanced Filters', () => {
	...
	it('should include events with no tags in search results when not filtering by tags', async () => {
		...
		const foundUntaggedEvent = body.events.find(
			(e: { id: string }) => e.id === eventWithoutTags.id
		)
	...
	describe('Popular events endpoint', () => {
		it('should return events sorted by popularity (attendance * 2 + likes)', async () => {
			...
			const morePopularIndex = body.events.findIndex(
				(e: { id: string }) => e.id === morePopular.id
			)
			const lessPopularIndex = body.events.findIndex(
				(e: { id: string }) => e.id === lessPopular.id
			)
	...
	describe('Platform Statistics Endpoint', () => {
		it('should return platform statistics for public events', async () => {
			...
			const todayEventStart =
				thirtyMinutesFromNow <= todayEnd ? thirtyMinutesFromNow : todayEnd
