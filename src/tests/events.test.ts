...
describe('Events API', () => {
	...
	describe('Event visibility', () => {
		...
		it('blocks access to private events for other users', async () => {
			...
			const res = await app.request(
				`/api/events/by-user/${testUser.username}/${privateEvent.id}`
			)
			expect(res.status).toBe(403)
		...
		it('allows followers to view follower-only events', async () => {
			...
			const res = await app.request(
				`/api/events/by-user/${testUser.username}/${followerOnlyEvent.id}`
			)
	...
	describe('GET /events/trending', () => {
		...
		it('handles tie-breaking when events have the same score', async () => {
			...
			if (indexA !== -1 && indexB !== -1) {
				...
				if (
					eventA_data &&
					eventB_data &&
					eventA_data.trendingScore === eventB_data.trendingScore
				) {
		...
		it('filters events correctly for authenticated users based on visibility', async () => {
			...
			const sessionSpyNonFollower = vi
				.spyOn(authModule.auth.api, 'getSession')
				.mockResolvedValue({
					user: {
						id: nonFollower.id,
			...
			const eventIdsNonFollower = bodyNonFollower.events.map(
	...
	describe('GET /events/by-user/:username/:eventId', () => {
		it('should handle remote users', async () => {
			...
			try {
				const res = await app.request(
					`/api/events/by-user/${remoteUser.username}/${event.id}`
				)
				expect(res.status).toBe(200)
			} finally {
		...
		it('should return 404 when event not found for user', async () => {
			const res = await app.request(
				`/api/events/by-user/${testUser.username}/nonexistent-event-id`
			)
			expect(res.status).toBe(404)
		})
	})
	...
	describe('Event Tags', () => {
		...
		it('should handle error when normalizeTags throws in create', async () => {
			...
			const normalizeTagsSpy = vi
				.spyOn(tagsModule, 'normalizeTags')
				.mockImplementation(() => {
	...
	describe('Location coordinate updates', () => {
		...
		it('should allow omitting both coordinates (no change)', async () => {
			...
			expect(body.locationLongitude).toBe(-74.006)
		})
	})
