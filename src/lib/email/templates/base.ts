...
export function BaseEmailTemplate({
	...
	const safeChildren = DOMPurify.sanitize(children)
	const safeFooterContent = footerContent ? DOMPurify.sanitize(footerContent) : null
	const previewSnippet = previewText
		? `<div style="display:none;max-height:0;overflow:hidden;color:transparent;">${DOMPurify.sanitize(previewText)}</div>`
		: ''
            ...
            <table role="presentation" border="0" cellpadding="0" cellspacing="0" style="${fullWidth ? 'width: 100%;' : 'display: inline-block;'}">
                <tr>
                    <td style="background-color: ${backgroundColor}; border: 1px solid ${borderColor}; border-radius: 8px; padding: 12px 24px; text-align: center;">
                        <a href="${safeHref}" style="color: ${textColor}; text-decoration: none; font-weight: 600; font-size: 16px; display: inline-block; ${fullWidth ? 'width: 100%;' : ''}" aria-label="${safeChildren}">
                            ${safeChildren}
                        </a>
                    </td>
                </tr>
