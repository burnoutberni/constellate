...
export const buildSearchWhereClause = async (
...
function applyTagFilters(where: Record<string, unknown>, tags?: string, categories?: string) {
	...
	const inputCategories =
		categories && categories.trim() ? normalizeTags(categories.split(',')) : []
...
app.get('/nearby', async (c) => {
	try {
		...
		const lonDelta =
			radiusKm / (KM_PER_DEGREE * Math.max(Math.abs(cosLat), MIN_COS_LAT_THRESHOLD))
		...
		const multiplier =
			requestedLimit <= SMALL_LIMIT_THRESHOLD
				? SMALL_LIMIT_MULTIPLIER
				: LARGE_LIMIT_MULTIPLIER
		const searchBatchSize = Math.min(
			Math.ceil(requestedLimit * multiplier),
		...
		if (error instanceof ZodError) {
			return c.json(
				{ error: 'Invalid nearby search parameters', details: error.issues },
				400 as const
			)
		}
