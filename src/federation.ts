...
// Type guard to ensure an object is a non-null object (Record)
...
async function handleFollow(activity: FollowActivity): Promise<void> {
	...
	const shouldAutoAccept =
		(targetUser as unknown as { autoAcceptFollowers?: boolean }).autoAcceptFollowers ?? true
...
async function handleAcceptEvent(
	...
	if (typeof object === 'string') {
		objectUrl = object
	} else if (isNonNullObject(object) && 'id' in object) {
	...
	const event = await prisma.event.findFirst({
		where: {
			OR: [{ externalId: objectUrl }, { id: objectUrl.split('/').pop() }],
		},
	})
...
function extractEventProperties(event: ActivityPubEvent | Record<string, unknown>) {
	...
	const getSanitized = (val: unknown) => {
	...
	return {
		...
		eventName: getSanitized(eventObj.name) || '',
		eventSummary: getSanitized(eventObj.summary),
		eventContent: getSanitized(eventObj.content),
		...
		attributedTo: getSanitized(eventObj.attributedTo),
	}
}

async function findEventByReference(reference: string) {
	const eventIdCandidate = reference.split('/').pop()
	const conditions: Prisma.EventWhereInput[] = [{ externalId: reference }]
	if (eventIdCandidate) {
...
async function handleCreateNote(
	activity: CreateActivity,
	note: ActivityPubNote | Record<string, unknown>
): Promise<void> {
	...
	const event = await prisma.event.findFirst({
		where: {
			OR: [{ externalId: inReplyTo }, { id: inReplyTo.split('/').pop() }],
		},
	})
	...
	// Broadcast update
	const commentWithAuthor = comment as typeof comment & {
...
function getObjectId(object: DeleteActivity['object']): string {
	...
	if (isNonNullObject(object) && 'id' in object && typeof object.id === 'string') {
...
function getFormerType(object: DeleteActivity['object']): string | null {
	if (
		...
		return object.formerType as string
	}
	return null
}
...
async function handleUndo(activity: UndoActivity): Promise<void> {
	...
	if (!isNonNullObject(object) || !('type' in object)) {
		return
	}
...
async function handleUndoLike(
	...
	likeActivity: LikeActivity | Record<string, unknown>
): Promise<void> {
	const actorUrl = activity.actor
	let objectUrl: string
	if (isNonNullObject(likeActivity) && 'object' in likeActivity) {
...
async function handleUndoFollow(
	...
	const actorUrl = activity.actor
	let objectUrl: string
	if (isNonNullObject(followActivity) && 'object' in followActivity) {
		objectUrl =
			typeof followActivity.object === 'string' ? (followActivity.object as string) : ''
	} else {
		objectUrl = ''
	}
...
async function handleUndoAttendance(
	activity: UndoActivity | Record<string, unknown>,
	attendanceActivity: AcceptActivity | Record<string, unknown>
): Promise<void> {
	let actorUrl: string
	if (isNonNullObject(activity) && 'actor' in activity) {
		actorUrl = typeof activity.actor === 'string' ? (activity.actor as string) : ''
	} else {
		actorUrl = ''
	}
	let objectUrl: string
	if (isNonNullObject(attendanceActivity) && 'object' in attendanceActivity) {
		objectUrl =
			typeof attendanceActivity.object === 'string'
				? (attendanceActivity.object as string)
				: ''
	} else {
		objectUrl = ''
	}
...
async function handleTentativeAccept(activity: Activity | Record<string, unknown>): Promise<void> {
	...
	let objectUrl: string
	if (isNonNullObject(activity) && 'object' in activity) {
		objectUrl = typeof activity.object === 'string' ? (activity.object as string) : ''
	} else {
		objectUrl = ''
	}
	const activityId =
		isNonNullObject(activity) && 'id' in activity && typeof activity.id === 'string'
			? activity.id
			: null
...
function isFollowRejectObject(object: unknown) {
	return (
		...
		object !== null &&
		'type' in object &&
		(object as { type?: unknown }).type === ActivityType.FOLLOW
	)
}
