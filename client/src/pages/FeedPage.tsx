...
export function FeedPage() {
	...
	// Bolt: Memoize feed items and validation to avoid expensive parsing on every render
						...
						{feedItems.length === 0 ? (
							...
							feedItems.map(({ item, validated }) => {
								...
								switch (item.type) {
									...
									case 'onboarding': {
										const itemData = validated as z.infer<typeof SuggestedUsersSchema>
										return (
											<OnboardingHero key={key} suggestions={itemData.suggestions} />
										)
									}
									...
									case 'suggested_users': {
										const itemData = validated as z.infer<typeof SuggestedUsersSchema>
										return (
											<SuggestedUsersCard key={key} users={itemData.suggestions} />
										)
									}
