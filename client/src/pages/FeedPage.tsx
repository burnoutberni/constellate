...
export function FeedPage() {
	...
	// Bolt: Memoize feed content to prevent validation on every render
	const feedContent = useMemo(() => {
		...
		}).filter((item): item is { key: string, type: string, data: unknown } => item !== null)
	}, [data])
					...
					<div className="space-y-6" style={{ overflowAnchor: 'none' }}>
						...
						{feedContent.length === 0 ? (
							...
							feedContent.map((item) => {
								...
								switch (type) {
									case 'header': {
										const { title } = itemData as z.infer<typeof HeaderSchema>
										return (
									...
									case 'onboarding': {
										const suggestionData = itemData as z.infer<typeof SuggestedUsersSchema>
										return <OnboardingHero key={key} suggestions={suggestionData.suggestions} />
									}

									case 'suggested_users': {
										const suggestionData = itemData as z.infer<typeof SuggestedUsersSchema>
										return <SuggestedUsersCard key={key} users={suggestionData.suggestions} />
									}

									case 'trending_event': {
										const event = itemData as z.infer<typeof EventSchema>
										return (
											<div key={key} className="h-full">
												<EventCard event={event} isAuthenticated={Boolean(user)} />
											</div>
										)
									}

									case 'activity': {
										const activity = itemData as z.infer<typeof ActivitySchema>
										// For "Smart Agenda", we show the Event itself
										return (
											<div key={key} className="h-full">
												{activity.event && <EventCard event={activity.event} isAuthenticated={Boolean(user)} />}
											</div>
										)
									}
