...
export function FeedPage() {
	...
	// Memoize the flattened and validated list to prevent expensive Zod parsing on every render
		...
		<div className="min-h-screen bg-background-secondary">
			...
			<div className="max-w-6xl mx-auto py-8 px-4 flex gap-6">
				...
				<div className="flex-1 min-w-0 relative">
						...
						{processedItems.length === 0 ? (
							...
							processedItems.map((item) => {
								...
								switch (item.type) {
									...
									case 'onboarding': {
										const onboardingData = validated as z.infer<typeof SuggestedUsersSchema>
										return <OnboardingHero key={key} suggestions={onboardingData.suggestions} />
									}

									case 'suggested_users': {
										const userData = validated as z.infer<typeof SuggestedUsersSchema>
									...
									case 'activity': {
										...
										return (
											<div key={key} className="h-full">
												<EventCard event={activity.event} isAuthenticated={Boolean(user)} />
											</div>
										)
									}
					...
					{!hasNextPage && processedItems.length > 0 && (
