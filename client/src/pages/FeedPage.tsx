...
export function FeedPage() {
	...
	// Bolt: Memoize feed content to avoid re-running Zod validation on every render.
	// This also ensures that we pass stable object references to child components like EventCard,
	// allowing React.memo to work effectively.
		...
		return items.map((item: FeedItem) => {
			...
			switch (item.type) {
				case 'header': {
					...
					return validated ? { type: 'header' as const, key, data: validated } : null
				}
				case 'onboarding': {
					const validated = getValidatedData(SuggestedUsersSchema, item.data, 'onboarding')
					return validated ? { type: 'onboarding' as const, key, data: validated } : null
				}
				case 'suggested_users': {
					const validated = getValidatedData(SuggestedUsersSchema, item.data, 'suggested_users')
					return validated ? { type: 'suggested_users' as const, key, data: validated } : null
				}
				case 'trending_event': {
					const validated = getValidatedData(EventSchema, item.data, 'trending_event')
					return validated ? { type: 'trending_event' as const, key, data: validated } : null
				}
				case 'activity': {
					const validated = getValidatedData(ActivitySchema, item.data, 'activity')
					return validated ? { type: 'activity' as const, key, data: validated } : null
				}
				default:
					return null
			}
