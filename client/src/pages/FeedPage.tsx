...
export function FeedPage() {
	...
	const { data: recommendedData, isLoading: recommendationsLoading } = useRecommendedEvents(5, {
		enabled: Boolean(user),
	})
	const { openCreateEventModal, closeCreateEventModal, createEventModalOpen, sseConnected } =
		useUIStore()
	...
	const trendingBusy =
		activeTab === 'trending' && (trendingLoading || (trendingFetching && !trendingData))
		...
		if (trendingError) {
			return (
				<Card variant="default" padding="md" className="text-center space-y-3">
					...
					<Button variant="primary" size="sm" onClick={() => refetchTrending()}>
						Try again
					</Button>
				</Card>
			)
									...
									{activeTab === 'activity' ? (
										...
										<>
											<span className="hidden sm:inline">
												{trendingUpdatedAt
													? `Updated ${trendingUpdatedAt.toLocaleTimeString(
															'en-US',
															{
																hour: 'numeric',
																minute: '2-digit',
											...
											<Button
												...
												aria-label={
													trendingFetching
														? 'Refreshing trending events'
														: 'Refresh trending events'
												}>
												Refresh
											</Button>
										</>
