...
export function FeedPage() {
	...
	// Memoize flattened and validated feed items to prevent expensive re-processing on every render
					...
					<div className="space-y-6" style={{ overflowAnchor: 'none' }}>
						...
						{feedItems.length === 0 ? (
							...
							feedItems.map((item) => {
								switch (item.type) {
									case 'header': {
										const validated = item.validatedData as z.infer<typeof HeaderSchema>
									...
									case 'onboarding': {
										const validated = item.validatedData as z.infer<typeof SuggestedUsersSchema>
										return <OnboardingHero key={item.key} suggestions={validated.suggestions} />
									}

									case 'suggested_users': {
										const validated = item.validatedData as z.infer<typeof SuggestedUsersSchema>
										return <SuggestedUsersCard key={item.key} users={validated.suggestions} />
									}

									case 'trending_event': {
										const validated = item.validatedData as z.infer<typeof EventSchema>
										return (
									...
									case 'activity': {
										const validated = item.validatedData as z.infer<typeof ActivitySchema>
