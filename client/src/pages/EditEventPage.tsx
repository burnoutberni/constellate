...
export function EditEventPage() {
	...
	const { suggestions: locationSuggestions } = useLocationSuggestions(locationSearch, true)
	...
	useEffect(() => {
		...
		if (user?.id !== event.user?.id) {
			handleError(
				new Error('You do not have permission to edit this event.'),
				'Permission denied',
	...
	const handleUseCurrentLocation = () => {
		if (!navigator.geolocation) {
			handleError(
				...
				{ context: 'EditEventPage.handleUseCurrentLocation' }
			)
			return
		}
		setGeoLoading(true)
	...
	const handleSubmit = async (e: FormEvent) => {
		...
		const locationLatitude =
			'latitude' in coordinatesResult ? coordinatesResult.latitude : undefined
		const locationLongitude =
			'longitude' in coordinatesResult ? coordinatesResult.longitude : undefined
					...
					<CardContent>
						...
						<form onSubmit={handleSubmit} className="space-y-6">
							...
							<div>
								...
								/>
								{locationSearch.length >= MIN_QUERY_LENGTH &&
