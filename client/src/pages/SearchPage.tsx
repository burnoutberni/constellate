...
export function SearchPage() {
	...
	const appliedFilters = useMemo<ActiveFilterChip[]>(() => {
		...
		if (dateRange && dateRange !== 'custom') {
			chips.push({
				key: 'dateRange',
				label: 'Date',
				value: DATE_RANGE_LABELS[dateRange] ?? dateRange,
			})
		}
	...
	const handleRemoveFilter = (chip: ActiveFilterChip) => {
		...
		if ((chip.key === 'categories' || chip.key === 'tags') && chip.removableValue) {
			const remaining = parseCommaList(next.get(chip.key)).filter(
				(item) => item !== chip.removableValue
			)
	...
	const renderEventResults = () => {
		...
		if (data?.events) {
			return data.events.map((event: Event) => (
				<EventResultCard key={event.id} event={event} formatter={eventDateTimeFormatter} />
			))
		}
		return null
	}
		...
		<div className="min-h-screen bg-neutral-100">
			...
			<div className="max-w-7xl mx-auto px-4 py-6 grid gap-6 lg:grid-cols-12">
				<div className="lg:col-span-4 space-y-4">
					...
					{!user && <TrendingEvents limit={5} windowDays={7} />}

					{/* Recommended Events Sidebar */}
					{user && <RecommendedEvents limit={5} />}
				</div>
		...
		<div className="card p-5 space-y-3">
			...
			<div className="flex flex-wrap items-center gap-4 text-sm text-neutral-500">
				...
				{typeof event._count?.likes === 'number' && (
					<span>❤️ {event._count.likes} likes</span>
				)}
