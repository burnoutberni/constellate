...
function MonthView({
	...
	// Bolt: Optimized from O(N*M) nested loop to O(N) single pass using a Map
...
function WeekView({
	...
	const eventsByDayAndHour = useMemo(() => {
		...
		const validDayStrings = new Set(weekDays.map((d) => d.toISOString()))
		...
		for (const event of events) {
			...
			if (validHours.has(hour)) {
				...
				const dayString = dayStart.toISOString()
				if (validDayStrings.has(dayString)) {
					const key = `${dayString}-${hour}`
