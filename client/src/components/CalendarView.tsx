...
function WeekView({
	...
	const eventsByDayAndHour = useMemo(() => {
		...
		for (const event of events) {
			...
			// Bolt: Optimization - iterate events once instead of nested loops
			const day = new Date(
			...
			// Use key format matching the render loop: ISO string of midnight date + hour
...
function DayView({
	...
	const eventsByHour = useMemo(() => {
		...
		for (const event of events) {
			...
			// Ensure event is on the current day
			if (
			...
			const hour = eventDate.getHours()
			// Bolt: Optimization - iterate events once.
			if (!map.has(hour)) {
				map.set(hour, [])
			}
