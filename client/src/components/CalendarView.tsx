...
function MonthView({
	...
	const eventsByDay = useMemo(() => {
		...
		// Bolt: Optimized from O(Days * N) to O(N) by iterating events once
...
function WeekView({
	...
	const eventsByDayAndHour = useMemo(() => {
		...
		// Bolt: Optimized from O(Slots * N) to O(N) by iterating events once
		for (const event of events) {
			const eventDate = new Date(event.startTime)
			const hour = eventDate.getHours()

			// Normalize to midnight to match weekDays logic
			const day = new Date(
...
function DayView({
	...
	const eventsByHour = useMemo(() => {
		...
		// Bolt: Optimized from O(Hours * N) to O(N) by iterating events once
		for (const event of events) {
			const eventDate = new Date(event.startTime)
