...
export const Section = React.forwardRef<HTMLElement, SectionProps>((props, ref) => {
	...
	const sectionClasses = cn(baseStyles, variantStyles[variant], paddingStyles[padding], className)

	// Determine content - wrap in Container if contained
	const content = contained ? <Container size={containerSize}>{children}</Container> : children
