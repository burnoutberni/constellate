        ...
        <div className="mb-6">
            ...
            {showAvatars ? (
                <div className="space-y-4">
                    ...
                    {attending.length > 0 && (
                        <div>
                            <h4 className="text-sm font-semibold text-text-secondary mb-2">Going</h4>
                            <div className="flex flex-wrap gap-3">
                                {(showAll ? attending : attending.slice(0, initialDisplayCount)).map((a) => (
                                    <Link
                                        key={a.user.id}
                                        to={`/@${a.user.username}`}
                    ...
                    {maybe.length > 0 && (
                        <div>
                            <h4 className="text-sm font-semibold text-text-secondary mb-2">Maybe</h4>
                            <div className="flex flex-wrap gap-3">
                                {(showAll ? maybe : maybe.slice(0, Math.max(1, initialDisplayCount - attending.length))).map((a) => (
                                    <Link
                                        key={a.user.id}
                                        to={`/@${a.user.username}`}
                                        className="flex items-center gap-2 hover:opacity-80 transition-opacity"
