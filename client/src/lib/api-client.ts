...
class ApiClient {
     ...
     */
    private async handleError(response: Response, baseMessage: string = 'Request failed'): Promise<never> {
    ...
    private async request<T>(
        ...
        try {
            ...
            if (interceptedResponse.status === 204 || interceptedResponse.headers.get('content-length') === '0') {
                return {} as T
            }
    ...
    async get<T>(
        ...
        return this.request<T>(endpoint, { ...options, method: 'GET' }, baseErrorMessage, queryParams)
    }
