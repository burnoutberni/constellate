...
import { TemplateCard, type EventTemplate } from '../../components/TemplateCard'
...
describe('TemplateCard', () => {
	...
	it('should call onDelete when confirmation is accepted', async () => {
		const { onDelete } = renderTemplateCard()
		const deleteButton = screen.getByText('Delete')

		// Click delete button and wait for confirmation dialog
		await act(async () => {
			fireEvent.click(deleteButton)
		})
	...
	it('should hide delete confirmation when Cancel is clicked', async () => {
		...
		await waitFor(() => {
			expect(
				screen.queryByText(/Are you sure you want to delete this template/)
