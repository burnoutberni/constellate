...
describe('DataExportSettings Component', () => {
	...
	it('should poll for export status and download when completed', async () => {
		...
		// Mock the status polling - first call returns PROCESSING, then COMPLETED
		mockApiGet
		...
		// Render first (before setting up spies that might interfere)
		render(<DataExportSettings />, { wrapper })
		...
		// Wait for first poll (polling starts after export is created, interval is 5000ms)
		await waitFor(
			() => {
		...
		// Wait for second poll (returns COMPLETED and triggers download)
		await waitFor(
			() => {
				expect(mockApiGet).toHaveBeenCalledWith(
					'/users/me/export/export-123',
	...
	}, 15000) // 15 second timeout for this test (needs 2 polling cycles = ~10 seconds)

	it('should show error toast when export fails', async () => {
		// Ensure clean state - clear any previous mocks
		vi.clearAllMocks()
		mockAddToast.mockClear()
		mockHandleError.mockClear()
		...
		// Wait for poll and error handling (polling interval is 5000ms)
		await waitFor(
			() => {
				expect(mockApiGet).toHaveBeenCalledWith(
					'/users/me/export/export-123',
		...
		// Verify the error status is displayed in the UI
		await waitFor(
			() => {
