...
export function useUserReminders() {
    return useQuery<UserRemindersResponse>({
        ...
        queryFn: () => api.get<UserRemindersResponse>('/users/me/reminders', undefined, undefined, 'Failed to fetch reminders'),
    })
}
...
export function useDeleteReminder(eventId: string) {
    ...
    return useMutation({
        mutationFn: (reminderId: string) => api.delete(`/events/${eventId}/reminders/${reminderId}`, undefined, 'Failed to delete reminder'),
        onError: (error) => {
