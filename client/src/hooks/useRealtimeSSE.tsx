...
const isValidEventData = (data: unknown, eventId: string): data is EventDetail & { id: string } => data !== null && typeof data === 'object' && 'id' in data && data.id === eventId
...
return
    ...
    eventSource.addEventListener('follow:removed', (e) => {
        ...
        if (event.data?.username) {
            queryClient.setQueryData(
                queryKeysParam.users.followStatus(event.data.username),
                {
                    isFollowing: false,
                    isAccepted: false,
                },
