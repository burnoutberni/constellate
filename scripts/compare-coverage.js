...
function main() {
	...
	console.log('----------------|-------------|-------------|----------')
	console.log(
		`Statements      | ${baseCoverage.statements.toFixed(2)}%      | ${prCoverage.statements.toFixed(2)}%      | ${(prCoverage.statements - baseCoverage.statements).toFixed(2)}%`
	)
	console.log(
		`Branches        | ${baseCoverage.branches.toFixed(2)}%      | ${prCoverage.branches.toFixed(2)}%      | ${(prCoverage.branches - baseCoverage.branches).toFixed(2)}%`
	)
	console.log(
		`Functions       | ${baseCoverage.functions.toFixed(2)}%      | ${prCoverage.functions.toFixed(2)}%      | ${(prCoverage.functions - baseCoverage.functions).toFixed(2)}%`
	)
	console.log(
		`Lines           | ${baseCoverage.lines.toFixed(2)}%      | ${prCoverage.lines.toFixed(2)}%      | ${(prCoverage.lines - baseCoverage.lines).toFixed(2)}%`
	)
	console.log('----------------|-------------|-------------|----------')
	console.log(
		`Overall         | ${baseOverall.toFixed(2)}%      | ${prOverall.toFixed(2)}%      | ${(prOverall - baseOverall).toFixed(2)}%`
	)
	console.log('')
	...
	if (prCoverage.statements < baseCoverage.statements - significantDecrease) {
		console.error(
			`❌ Statement coverage decreased by ${(baseCoverage.statements - prCoverage.statements).toFixed(2)}%`
		)
		hasSignificantDecrease = true
	}
	if (prCoverage.branches < baseCoverage.branches - significantDecrease) {
		console.error(
			`❌ Branch coverage decreased by ${(baseCoverage.branches - prCoverage.branches).toFixed(2)}%`
		)
		hasSignificantDecrease = true
	}
	if (prCoverage.functions < baseCoverage.functions - significantDecrease) {
		console.error(
			`❌ Function coverage decreased by ${(baseCoverage.functions - prCoverage.functions).toFixed(2)}%`
		)
		hasSignificantDecrease = true
	}
	if (prCoverage.lines < baseCoverage.lines - significantDecrease) {
		console.error(
			`❌ Line coverage decreased by ${(baseCoverage.lines - prCoverage.lines).toFixed(2)}%`
		)
		hasSignificantDecrease = true
	}
